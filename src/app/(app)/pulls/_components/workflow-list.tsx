...
> = {
  pack: {
    ...
    actionBusy: "Packing…",
    actionBtnCls: "bg-sky-500 text-zinc-950",
  },
  send: {
    emptyTitle:
    ...
    actionBusy: "Sending…",
    actionBtnCls: "bg-blue-500 text-zinc-950",
  },
};
      ...
      {groups.map((g) => {
        ...
        return (
          <section key={g.key} className={`border-l-4 ${color.border}`}>
            <header className="px-4 py-2 bg-zinc-950/80 sticky top-0 backdrop-blur border-b border-zinc-900 z-[1]">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <span
                    className={`shrink-0 text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full ${color.badge}`}
                  >
                      ...
                      <div className="flex gap-2 mt-2">
                        {isWarehouseHandoff ? (
                          ...
                          <button
                            ...
                            disabled={busy !== null}
                            className={`text-xs px-3 h-8 rounded-full font-medium disabled:opacity-50 ${copy.actionBtnCls}`}
                          >
